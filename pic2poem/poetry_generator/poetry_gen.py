...
poetry_file = os.path.abspath(os.path.join(os.getcwd(), "pic2poem/poetry_generator/poetry.txt"))
#save_dir = "./poetry_generator/log"
save_dir = os.path.abspath(os.path.join(os.getcwd(), "pic2poem/poetry_generator/log"))
...
def main():
    ...
    if args.mode == 'sample':
        ...
        poem = sample(data, model, head=args.head)
        print(poem)
        f = open('poem1.txt', 'a')
        f.writelines("\n" + poem)
        f.close()
...
def generate(hint):
    ...
    data = Data()
    model = Model(data=data, infer=infer)
    print(sample(data, model, head=hint))
